...
<div class="z-wrap"><form class="z-form" onchange="form_update(event)" onsubmit="return false"><div><label for="2611c310" title>发行版</label><select id="2611c310" name="deb_release" title><option value="debian">Debian</option><option value="ubuntu">Ubuntu</option><option value="raspbian">Raspbian</option></select></div></form><pre class="z-code"></pre></div><tmpl z-input="deb_release" z-lang="bash">
...
<p>安装依赖，下载 repo 文件，并把软件仓库地址替换为镜像站：</p>
<div class="z-wrap"><form class="z-form" onchange="form_update(event)" onsubmit="return false"><div><label for="ff6995c3" title>发行版</label><select id="ff6995c3" name="rh_release" title><option value="centos">CentOS</option><option value="rhel">RHEL</option></select></div></form><pre class="z-code"></pre></div><tmpl z-input="rh_release" z-lang="bash">
